...
3. [Normalizing Flows](#NormFlows)  
4. [Inference with Normalizing Flows](#InfNormFlow)  
5. [Algorithm of Normalizing Flows](#AlgoNormFlows) 
6. [Bayes Flow](#BFlowStart)  
7. [Algorithm of Bayes Flow](#AlgoBayesFlow)
8. [Recent Developments in Normalizing Flows](#DevelopNormFlow)
9. [Summary of Normalizing Flow Families](#SummNormFlow)
10. [Future Directions of Normalizing Flows](#FutureDir)
...
- Many real-world distributions are **non-Gaussian**, **skewed or heavy-tailed**, **multi-modal and correlated**
- Classical models (Gaussian, mixtures) can be **too restrictive**
- We want models that are: **Flexible**, **Exact likelihood**, **Efficient to train and sample**
...
- **Expansions**  
  → The map $z'=f(z)$ pulls the pointz $z$ away from a region in $\mathbb{R}^d$, reducing density in that region.

- **Contractions**  
  → concentrate mass, increase density locally
...
langevin_vs_hamiltonian_story4 = {
  ...
  const friction = Inputs.range([0, 0.25], { label: "Hamiltonian friction (0≈HMC ideal)", step: 0.01, value: 0.03 });

  // NEW: cycle length for "uphill → downhill" repeats
...
<div class="flow-controls" style="max-width:${W}px;">
  ...
  gL.append("text")
    ...
    .text("drift = −∇U(z) (downhill), noise = shake  → repeated resets show “roll down”");

  gR.append("text")
    .attr("x", 14).attr("y", 46)
    .attr("font-size", 12).attr("opacity", 0.70)
    .text("leapfrog on U(z) with momentum p (inertia) → long, smooth descents");
  ...
  // Strong bowl so “downhill” is visually clear; high density ~ exp(-U)
  function U(x, y) {
  ...
  function render() {
    ...
    // Langevin drift arrows: -∇U (downhill)
  ...
  function step(dt) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const driftStrength = 1.25; // strong inward drift so “uphill→downhill” is obvious
  ...
  // This creates: uphill → downhill, then reset uphill → downhill, ...
  // -------------------------------------------------
  let last = null;
  ...
  function tick(ts) {
    ...
    if (phase >= T) {
      phase = 0;
      resetAll();      // <-- this gives the repeated “uphill → downhill” behavior
    }
...
- Jacobian determinant computed in **$O(D)$** and total cost scales **linearly in flow length** $K$
- No matrix inverses required and is compatible with minibatch SGD and backpropagation
...
bayesflow_algo_anim575 = {
  ...
  const train = [
    ...
    {id:"sim", x:570,  y:yT, w:230, h:92, title:"3) Simulate x", line1:"ξ_i ~ p(ξ), x_i = g(θ^(m), ξ_i)", line2:"for i = 1..N"},
    {id:"sum", x:840,  y:yT, w:210, h:92, title:"4) Summary",    line1:"x̃ = hψ(x1:N)"},
  ...
  const infer = [
    ...
    {id:"sumo",x:360,  y:yI, w:210, h:92, title:"Summary",         line1:"x̃ = hψ(x1:N)"},
    {id:"z",   x:610,  y:yI, w:210, h:92, title:"Sample latent",   line1:"z(l) ~ N(0, I)"},
    {id:"inv", x:860,  y:yI, w:250, h:92, title:"Flow (←)",        line1:"θ^(l) = fϕ(z(l); x̃)"},
    // tweak return to be explicit (optional)
    {id:"out", x:1150, y:yI, w:160, h:92, title:"Return",          line1:"θ^(l) for L = 1..L"},
  ];
...
Flows can be defined as **continuous dynamics**: $\quad \quad \frac{dx(t)}{dt} = F(x(t), t)$
...
| Coupling flows | $z_{1:d}=x_{1:d} \quad$ $z_{d+1:D}=x_{d+1:D}\odot e^{s(x_{1:d})}+t(x_{1:d})$ | Split variables, conditional transforms | Exact inverse, scalable | Needs good coupling design |
